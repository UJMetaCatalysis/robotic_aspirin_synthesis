...
hotplate = pl.RadleysCarouselConnect('hotplate', 'serial', None, "COM")
hotplate.initialize_device()
...
while True:
    ...
    if response == "S":
        ...
        hotplate.set_temperature(70)
        hotplate.start_temperature_regulation()
        last_time = time.time()
        temp = hotplate.get_temperature()
        while temp < 70:
            if time.time() - last_time > 5:
                temp = hotplate.get_temperature()
                last_time = time.time()
        hotplate.set_speed(500)
        ...
        with Bar('   Heating', fill='#', suffix="%(percent).1f%%") as bar:
            while time_elapsed < heat_time + 2:
                ...
                time.sleep(0.1)
                time_elapsed = time.time() - start_time
        hotplate.stop_temperature_regulation()
